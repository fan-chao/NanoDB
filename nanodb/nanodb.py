...
from .utils import print_table, tqdm_redirect_stdout, convert_dtype


class NanoDB:
    def __init__(self, path=None, model="openai/clip-vit-large-patch14-336", dtype=np.float16, autosave=False,
                 **kwargs):
        """
        kwargs:
        ...
        if model:
            ...
            self.model = CLIPModel(model, dtype=convert_dtype(dtype, to='pt'), **kwargs)
    ...
    def search(self, query, k=4):
        ...
        """
        if isinstance(query, (np.ndarray, torch.Tensor)) and len(query.shape) == 2 and query.shape[1] == \
                self.index.shape[1]:
            embedding = query
        else:
    ...
    def add(self, data, metadata=None, **kwargs):
        if isinstance(data, (np.ndarray, torch.Tensor)) and len(data.shape) == 2 and data.shape[1] == self.index.shape[
            1]:
            embedding = data
        else:
    ...
    def scan(self, path, max_items=None, **kwargs):
        ...
        logging.info(
            f"added {len(indexes)} items to the index in from {path} ({time_elapsed:.1f} sec, {len(indexes) / time_elapsed:.1f} items/sec)")
    ...
    def load(self, path=None):
        ...
        logging.info(f"loading nanodb database from {path}")
        ...
        logging.success(
            f"nanodb loaded {self.index.shape} records, {self.index.size()} bytes in {time.perf_counter() - time_begin:.2f} sec")
    ...
    def save(self, path=None):
        ...
        logging.info(f"saving nanodb database to {path}")
        ...
        logging.success(
            f"nanodb wrote {self.index.shape} records, {self.index.size()} bytes in {time.perf_counter() - time_begin:.2f} sec")
    ...
    def embed(self, data, type=None, **kwargs):
        ...
        if type == 'image':
            ...
            embedding = self.model.text.embed_text(data)
            print_table(self.model.text.stats)
        else:
    ...
    def test(self, k):
        for i in tqdm.tqdm(range(len(self.index)), file=sys.stdout):
            with tqdm_redirect_stdout():
                ...
                for n in range(k):
                    print(
                        f"   * {indexes[n]} {self.metadata[indexes[n]]['path']}  {'similarity' if self.index.metric == 'cosine' else 'distance'}={distances[n]}")
